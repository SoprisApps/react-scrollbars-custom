...
const AXIS_DIRECTION_PROP_TYPE = PropTypes.oneOf([AXIS_DIRECTION.X, AXIS_DIRECTION.Y]);
...
const TRACK_CLICK_BEHAVIOR_PROP_TYPE = PropTypes.oneOf([TRACK_CLICK_BEHAVIOR.JUMP, TRACK_CLICK_BEHAVIOR.STEP]);
  ...
  constructor(props) {
    ...
    this.handleTrackXClick = (ev, values) => {
      ...
      if (this.props.trackClickBehavior === TRACK_CLICK_BEHAVIOR.STEP) {
        target = (
          this.scrollValues.isRTL ? this.scrollValues.scrollLeft > target : this.scrollValues.scrollLeft < target
        )
